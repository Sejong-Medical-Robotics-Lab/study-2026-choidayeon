...
│   │   ├── param_sweep.py     # 보행 파라미터 경계 스윕(헤드리스, 완주/낙상 자동 판정)
│   │   └── mission7_judge.py  # 확정 파라미터 세트 N회 반복 판정 + CSV/비교표(md) 생성
│   ├── g1-real/           # G1 실기체 제어/모니터링 스크립트 (하위 저장소)
│   │   ├── g1_real_monitor.py   # 실기체 관절 상태 읽기 전용 모니터링(precheck/baseline/watch)
│   │   └── g1_real_sequence.py  # 상체 동작 시퀀스 실기체 실행기 (SOP 게이트 포함)
│   ├── 5page_mission3.py  # 상체 시퀀스 설계표를 옮긴 뼈대 코드(호출부 주석 처리)
│   ├── 5page_mission4.py  # g1edu(G1Sim/LocoClient)로 실제 동작하는 완성판
│   ├── 관절지도            # 다리/허리/팔/IMU 자유도 구성 정리
│   ├── *.md               # 미션별 실습 정리 (지지영역, 상태전이, 상태전이판졀, 실패경로추적, 파라미터, 모니터링콜규약)
│   └── 상체시퀀스 설계     # 상체 동작 시퀀스 설계 표(순서·명령·대기·기대상태·실패경로)
...
- G1 안전 수칙 3가지: ① 행어 미거치 단독 기립 금지 ② 멘토 포함 2인 이상(1인은 비상정지 전담) ③ 보행 시 반경 2m 확보
- 관절 구성 지도(`관절지도`): 다리×2(고관절 roll·pitch·yaw 3 + 무릎 1 + 발목 pitch·roll 2 = 다리당 6DOF) + 허리(회전·기울임) + 팔×2(제스처·모션용) + IMU·내장 컴퓨터(몸통 기울기·각속도)로 정리
- 균형 제어 원리 학습: support polygon(지지 영역) 위에 무게중심을 유지해야 하고, 벗어나면 넘어짐이 시작된다는 것 — 작은 흔들림은 자세 보상(발목·허리 미세 조정), 큰 흔들림은 스텝(발을 옮겨 지지 영역 자체를 이동)으로 대응
- 지지 영역 체감 실험(`지지영역.md`): 두 발 벌리기/한 발로 서기/상체 앞으로 기울이기를 직접 해보며 지지 영역(두 발 사이 면적)-무게중심-자세 보상-스텝 전환의 관계를 몸으로 확인
- 거치 → 준비 → 기립 → 보행 순서와 `Damp()`를 기준점("시작은 Damp, 이상 시 즉시 Damp")으로 한 안전 절차 이해 — "엎드린 로봇에 보행 명령"이 왜 무의미한지 원리로 이해
...
- 상체 동작 시퀀스 설계(`상체시퀀스 설계`): 순서·명령어·대기시간·기대 상태·실패 시 경로를 표로 미리 설계 — ①Damp(1.0s) ②StandUp(모드가 `balance_stand`가 될 때까지 대기+1.0s 안정화) ③wave(오른팔 인사) ④hands_up/bow(왼팔·양손 인사) ⑤복귀/Damp(finally 블록) 5단계, 각 단계 실패 시 전부 `except`/`finally`로 Damp 복귀
  - `5page_mission3.py`: 위 설계표를 그대로 옮긴 뼈대 코드 — 실제 클라이언트 호출은 주석 처리한 채 6단계(초기화→상태확인→Damp→기립전이→모션실행→정리) 흐름과 `try/except/finally` 안전 구조만 먼저 검증
  - `5page_mission4.py`: `g1edu`(`G1Sim`/`LocoClient`)로 실제 동작하는 완성판 — `while client.GetMode() != "balance_stand"` 로 기립 완료를 폴링 대기한 뒤 `wave`→`hands_up`→`bow` 순서로 실행하고, 어떤 예외가 나도 `finally`에서 `Damp()`로 수렴
- MuJoCo 기반 시뮬레이션(`g1-edu`)에서 파라미터 조정을 통한 보행 균형 실험
  ...
  - `g1-edu/param_sweep.py`: 관찰표 작성 전 경계를 헤드리스로 미리 훑어보는 보조 스크립트 — vx/step_period/step_height/com_shift/vyaw를 "한 번에 하나만" 바꾸는 기본 스윕 12종 + `--set key=val`로 단일 시도 지정, 각 시도의 완주/낙상 여부·낙상 시각·실측 평균속도를 표로 출력
  - `g1-edu/mission7_judge.py`: 관찰표로 확정한 "안정 세트"/"속달 세트" 두 파라미터 조합을 같은 조건으로 N회(기본 3회) 반복 실행해 완주율을 자동 판정 — 결과를 CSV + 비교표(md)로 저장하고, "근거(관찰표 행)"·"한 줄 해석" 칸은 사람이 채우도록 의도적으로 비워둠
- 실기체(`g1-real`) 제어 스크립트 작성 — 시뮬 검증이 끝난 설계를 실기체로 그대로 옮기되, 곳곳에 게이트(사람 확인)를 박아 넣은 것이 핵심
  - `g1_real_monitor.py`: 로봇에 명령을 보내지 않는 읽기 전용 모니터링 스테이션. `precheck`(조작 전 상태 스트림 Hz·IMU 기울기·모터 온도·토크 수준을 점검해 "상태 정상"/"점검 필요" 콜 자동 판정) / `baseline`(조용한 상태에서 관절별 tau·온도·IMU 기준선을 json으로 저장) / `watch`(기준선 대비 실시간 감시 — 토크 이상·기울기 증가·모니터링 끊김을 콜 문구 그대로 출력 + CSV 로그) 3개 모드
  - `g1_real_sequence.py`: 상체 시퀀스 설계표를 실기체에서 실행하는 스크립트. `SEQUENCE`에는 상체 동작 행만 적을 수 있고 `move`/`stop`/`standup`/`damp`는 학생 코드에서 아예 금지(전이는 프레임이, 보행은 멘토가 전담) — 실행 전 SOP 게이트 4종(5단계 체크·행어 거치 확인·멘토 리모컨 소지·모니터링 담당 가동) 통과가 필수이고, `--dry-run`으로 로봇 없이 설계만 리뷰 가능, 종료 시 한 줄 로그(csv)를 자동 기록
